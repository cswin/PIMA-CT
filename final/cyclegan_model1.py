...
class CycleGANModel():
    ...
    def __init__(self, opt):
        ...
        GB_model = DnCNN()
        GB_model.to(self.gpu_ids[0])
        GB_model = torch.nn.DataParallel(GB_model, self.gpu_ids)  # multi-GPUs
        self.netGd = GB_model
        ...
        self.optimizer_D = torch.optim.Adam(itertools.chain(self.netDs.parameters(), self.netDd.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
        self.schedulerD = MultiStepLR(self.optimizer_D, milestones=[10, 20, 30, 40, 50, 60, 70], gamma=0.2)
        self.schedulerG = MultiStepLR(self.optimizer_G, milestones=[10, 20, 30, 40, 50, 60, 70], gamma=0.2)
    ...
    def backward_G(self):
        ...
        self.loss_G1 = self.loss_Gs + self.loss_cycle_A
        self.loss_G1.backward()

        self.optimizer_G.step()
        self.optimizer_G.zero_grad()

        self.loss_G2 = self.loss_Gd + self.loss_Gd_L1 + self.loss_cycle_B

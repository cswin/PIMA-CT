...
parser.add_argument('--model', default='unet64_basic_origincyclegan_noDN_l1loss', type=str, help='choose a type of model')
...
parser.add_argument('--datasetA', default='../dataset/CT_Data_All_Patients/train', type=str, help='path of high-dose data')
...
parser.add_argument('--lambda_DN', type=float, default=0.0, help='weight for DN(A) and fakeB')
...
parser.add_argument('--pretrainA', default='models/pretrainGA.pth', type=str)
parser.add_argument('--pretrainB', default='models/pretrainGB.pth', type=str)
...
if args.datatype == 'aligned':
    ...
    dataA, dataB = dg.unaligned_datagenerator(args.datasetA, args.datasetB, args.batch_size, args.aug_times, args.patch_size, args.stride, args.threshold)
...
for epoch in range(args.epoch):
    for i, data in enumerate(DLoader):
        ...
        print('[%d/%d][%d/%d] Loss_DA: %.4f Loss_DB: %.4f Loss_G: %.4f Loss_GA: %.4f Loss_GB: %.4f CycleA: %.4f CycleB: %.4f' % (epoch, args.epoch, i, len(DLoader), model.loss_D_A, model.loss_D_B, model.loss_G, model.loss_G_A, model.loss_G_B, model.loss_cycle_A, model.loss_cycle_B))

...
class CycleGANModel():
    ...
    def load_networks_teacher_stu(self, opt):
        ...
        D_A_dir = os.path.join(opt.model_dir, opt.model + '_D_A')
        D_B_dir = os.path.join(opt.model_dir, opt.model + '_D_B')
        initial_time, initial_epoch = findLastCheckpoint_time_epoch(save_dir=G_A_dir)  # load the last model in matconvnet style
        if initial_epoch > 0:
            print('resuming by loading times %03d epoch %03d' % (initial_time+1, initial_epoch))
            self.netG_A.load_state_dict(torch.load(os.path.join(G_A_dir, 'model_%03d_%03d.pth' % (initial_time+1, initial_epoch))))
        else:
            print('resuming by loading pretrainGA.pth')
            self.netG_A.load_state_dict(torch.load(opt.pretrainGA))

        initial_time, initial_epoch = findLastCheckpoint_time_epoch(save_dir=G_B_dir)  # load the last model in matconvnet style
        if initial_epoch > 0:
            print('resuming by loading times %03d epoch %03d' % (initial_time+1, initial_epoch))
            self.netG_B.load_state_dict(torch.load(os.path.join(G_B_dir, 'model_%03d_%03d.pth' % (initial_time+1, initial_epoch))))
        else:
            print('resuming by loading times pretrainGB.pth')
            self.netG_B.load_state_dict(torch.load(opt.pretrainGB))

        initial_time, initial_epoch = findLastCheckpoint_time_epoch(save_dir=D_A_dir)  # load the last model in matconvnet style
        if initial_epoch > 0:
            print('resuming by loading times %03d epoch %03d' % (initial_time+1, initial_epoch))
            self.netD_A.load_state_dict(torch.load(os.path.join(D_A_dir, 'model_%03d_%03d.pth' % (initial_time+1, initial_epoch))))

        initial_time, initial_epoch = findLastCheckpoint_time_epoch(save_dir=D_B_dir)  # load the last model in matconvnet style
        if initial_epoch > 0:
            print('resuming by loading times %03d epoch %03d' % (initial_time+1, initial_epoch))
            self.netD_B.load_state_dict(torch.load(os.path.join(D_B_dir, 'model_%03d_%03d.pth' % (initial_time+1, initial_epoch))))
    ...
    def backward_G(self):
        ...
        if self.cyclemode == 0:
            ...
            # Forward cycle loss || G_B(G_A(A)) - A||
            self.loss_cycle_A = self.criterionCycle1(self.rec_A, self.real_A) * lambda_A
            # Backward cycle loss || G_A(G_B(B)) - B||
            self.loss_cycle_B = self.criterionCycle1(self.rec_B, self.real_B) * lambda_B
        # combined loss and calculate gradients

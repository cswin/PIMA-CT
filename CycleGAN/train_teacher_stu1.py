...
parser.add_argument('--model', default='teacher_stu1_unet64_basic_iden_60_cycleAB', type=str, help='choose a type of model')
...
parser.add_argument('--dataset_unlabeled', default='../dataset/CT_Data_All_Patients/train', type=str, help='path of high-dose data without paired low-dose data')
parser.add_argument('--result_pseudo', default='../dataset/CT_Data_All_Patients/pseudo', type=str, help='path of pseudo dictionary')
...
if initial_time == 0:
    ...
    # netG_A
    netG = networks.define_G(args.input_nc, args.output_nc, args.ngf, args.netG, args.norm, not args.no_dropout, args.init_type, args.init_gain)
    if cuda:
        netG = netG.cuda()
        device_ids = [0]
    ...
    netG.load_state_dict(torch.load(args.pretrainGA))
...
pseudo_data = dg.aligned_datagenerator_bmp(args.dataset_unlabeled, args.result_pseudo, args.batch_size, args.aug_times, args.patch_size, args.stride, args.threshold)
pseudo_data = pseudo_data.astype('float32') / 255.0
# print(pseudo_data.shape)
pseudo_data = torch.from_numpy(pseudo_data.transpose((0, 1, 4, 2, 3)))  # tensor of the clean patches, N X C X H X W
DDataset = AlignedDenoisingDataset(pseudo_data)
...
for teach_time in range(initial_time, args.teach_nums):
    ...
    for epoch in range(initial_epoch, args.epoch):
        for i, data in enumerate(DLoader):
            ...
            if i % 20 == 0:
                print('[%d/%d][%d/%d] Loss_DA: %.4f Loss_DB: %.4f Loss_G: %.4f Loss_DN: %.4f Loss_GA: %.4f Loss_GB: %.4f Loss_cycleA: %.4f Loss_cycleB: %.4f Loss_idtA: %.4f Loss_idtB: %.4f' % (epoch, args.epoch, i, len(DLoader), model.loss_D_A, model.loss_D_B, model.loss_G, model.loss_DN, model.loss_G_A, model.loss_G_B, model.loss_cycle_A, model.loss_cycle_B, model.loss_idt_A, model.loss_idt_B))

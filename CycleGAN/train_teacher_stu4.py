...
# 把去噪的Generator改为DnCNN，数据只用病人的数据和模拟的高斯噪声
...
from cyclegan_model2 import CycleGANModel
import os
import cv2
import numpy as np
...
parser.add_argument('--model', default='teacher_stu_unet_dncnn_onlyPatient_l1', type=str, help='choose a type of model')
...
parser.add_argument('--dataset_noise_free', default='../dataset/CT_Data_All_Patients/train', type=str,
                    help='noise_free patient')
parser.add_argument('--dataset_noises', default='../dataset/noises', type=str, help='phantom low and patient simulation')
parser.add_argument('--phantom_low', default='../dataset/patients_noisy/Gua_sim_LD_20mAs/train', type=str,
                    help='path of low-dose data')

parser.add_argument('--phantom_high', default='../dataset/CT_Data_All_Patients/train', type=str,
                    help='path of high-dose data')

parser.add_argument('--result_pseudo', default='../dataset/CT_Data_All_Patients/pseudo20mAs', type=str,
...
for teach_time in range(initial_time, args.teach_nums):
    ...
    for im in os.listdir(args.dataset_unlabeled):
        if im.endswith(".tif") or im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
            ...
            save_result(x_, path=os.path.join(args.result_pseudo, name + ext))
